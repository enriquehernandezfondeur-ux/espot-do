...
export default function TimePicker({
  ...
  const displayLabel = SLOTS.find(s => s.value === value)?.label ?? null
    ...
    <div className="relative" ref={ref}>
      ...
      <button
        ...
        {value ? (
          ...
          <ChevronDown size={14}
            style={{ color: 'var(--text-muted)', flexShrink: 0, transition: 'transform 0.15s', transform: open ? 'rotate(180deg)' : 'none' }} />
        )}
      </button>

      {/* Dropdown — grid compacto */}
      {open && (
        <div
          ...
          {Object.keys(grouped).length === 0 ? (
            <div className="px-4 py-6 text-center">
              ...
              <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
                {minMinutesAfter
                  ? `Este espacio requiere mínimo ${Math.round(minMinutesAfter / 60)}h. Elige una hora de inicio más temprana.`
