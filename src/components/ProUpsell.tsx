...
export function ProUpsell({ title, children }: { title: string; children?: React.ReactNode }) {
  return (
    <Link
      ...
      style={{ background: 'rgba(53,196,147,0.07)', border: '1px solid var(--brand-border)' }}
    >
      <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: 'rgba(53,196,147,0.16)' }}>
        <Crown size={18} style={{ color: 'var(--brand)' }} />
      </div>
      ...
      <ArrowRight size={16} style={{ color: 'var(--brand)' }} className="shrink-0" />
    </Link>
  )
}
