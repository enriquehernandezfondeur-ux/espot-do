    ...
    <div className="light-theme min-h-dvh w-full" style={{ background: 'var(--bg-base)', color: 'var(--text-primary)' }}>
      ...
      <footer style={{ background: '#F4F6F5', borderTop: '1px solid #E0E7E3' }}>
        <style>{`
          ...
          .ft-pay:hover img { opacity: .85; }
        `}</style>
        <div className="max-w-7xl mx-auto px-5 md:px-6 py-12 md:py-14">

          {/* ── Bloque superior: marca + columnas de enlaces (grid 12-col reparte el ancho) ── */}
          <div className="grid gap-10 md:grid-cols-12 md:gap-8 md:items-start">

            {/* Marca → claim → redes → App Store */}
            <div className="md:col-span-5 lg:col-span-4 flex flex-col items-center md:items-start gap-5 text-center md:text-left">
              <div className="flex flex-col items-center md:items-start gap-2.5">
                <Link href="/" aria-label="Inicio Espot"><img src="/logo-dark.svg" alt="Espot" style={{height:26,width:'auto'}}/></Link>
                <p className="text-sm" style={{color:'#6B7280',lineHeight:1.6}}>Espacios a tu alcance.</p>
              </div>
              ...
              <div className="flex gap-2.5">
                {[
                  ...
                  {href:'https://www.linkedin.com/company/espotdo/',label:'LinkedIn',path:'M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 23.2 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z'},
                ].map(s=>(
                  <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={`Espot en ${s.label}`}
                    className="ft-soc w-10 h-10 rounded-xl flex items-center justify-center"
                    style={{background:'#E8EEE9',color:'#6B7280'}}>
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d={s.path}/></svg>
                  </a>
                ))}
              </div>

              {/* App Store — próximamente (la app aún no está publicada) */}
              <div className="flex flex-col items-center md:items-start gap-1.5">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/app-store-badge.png" aria-label="App de Espot, próximamente en App Store"
                  alt="Próximamente en App Store"
                  style={{height:40,width:'auto',display:'block',objectFit:'contain',opacity:.92}} />
                <span className="text-[11px] font-medium tracking-wide" style={{color:'#9CA3AF'}}>App para iOS · muy pronto</span>
              </div>
            </div>

            {/* Columnas de enlaces */}
            <div className="md:col-span-7 lg:col-span-8 grid grid-cols-3 gap-6 sm:gap-8 md:gap-10 text-center sm:text-left md:justify-items-end lg:justify-items-start">
              {[
                { title: 'Plataforma', links: [{href:'/buscar',label:'Explorar espacios'},{href:'/para-clientes',label:'Para clientes'},{href:'/para-propietarios',label:'Para propietarios'}] },
                { title: 'Anfitriones', links: [{href:'/para-propietarios',label:'Publicar espacio'},{href:'/dashboard/host',label:'Panel de control'},{href:'/contacto',label:'Contacto'}] },
          ...
          {/* ── Barra inferior: medios de pago + copyright ── */}
          <div className="mt-12 pt-7 flex flex-col items-center gap-6 sm:flex-row sm:justify-between"
            style={{borderTop:'1px solid #E0E7E3'}}>

            {/* Medios de pago y seguridad — monocromático, misma altura, 1 fila desktop / 2 móvil */}
            <div className="ft-pay flex items-center justify-center flex-wrap gap-x-6 gap-y-3.5 max-w-md sm:max-w-none">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/visa.png" alt="Visa" style={{height:20,width:'auto',display:'block',objectFit:'contain'}} />
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/mastercard.png" alt="Mastercard" style={{height:22,width:'auto',display:'block',objectFit:'contain'}} />
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/visa-secure.png" alt="Visa Secure" style={{height:24,width:'auto',display:'block',objectFit:'contain'}} />
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/mastercard-id-check.png" alt="Mastercard ID Check" style={{height:24,width:'auto',display:'block',objectFit:'contain'}} />
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/azul.jpg" alt="Procesado por Azul" style={{height:22,width:'auto',borderRadius:5,display:'block',objectFit:'contain'}} />
            </div>

            <p className="text-xs text-center sm:whitespace-nowrap" style={{color:'#9CA3AF'}}>© 2026 ESPOT, S.R.L. · República Dominicana</p>
          </div>
        </div>
      </footer>
    </div>
