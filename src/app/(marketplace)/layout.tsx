...
export default function MarketplaceLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="light-theme min-h-dvh w-full" style={{ background: 'var(--bg-base)', color: 'var(--text-primary)', overflowX: 'hidden' }}>
      ...
      {/* ── FOOTER ── */}
      <footer style={{ background: '#fff', borderTop: '1px solid #E8EEE9' }}>
        <div className="max-w-7xl mx-auto px-4 md:px-6">

          {/* Grid principal: 4 columnas iguales en desktop, 2 en tablet, 1 en mobile */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 py-10">

            {/* Col 1: Marketplace */}
            <div className="space-y-2.5">
              <p className="text-[10px] font-bold uppercase tracking-widest mb-4" style={{ color: '#35C493' }}>Marketplace</p>
              {[
                { href: '/buscar',            label: 'Explorar espacios' },
                ...
                <Link key={href} href={href} className="block text-sm link-muted" style={{ color: '#6B7280' }}>{label}</Link>
              ))}
            </div>

            {/* Col 2: Anfitriones */}
            <div className="space-y-2.5">
              <p className="text-[10px] font-bold uppercase tracking-widest mb-4" style={{ color: '#35C493' }}>Anfitriones</p>
              {[
                { href: '/para-propietarios', label: 'Publicar mi espacio' },
                { href: '/dashboard/host',    label: 'Panel de control' },
                { href: '/auth',              label: 'Iniciar sesión' },
              ].map(({ href, label }) => (
                <Link key={label} href={href} className="block text-sm link-muted" style={{ color: '#6B7280' }}>{label}</Link>
              ))}
            </div>

            {/* Col 3: Legal */}
            <div className="space-y-2.5">
              <p className="text-[10px] font-bold uppercase tracking-widest mb-4" style={{ color: '#35C493' }}>Legal</p>
              {[
            ...
            {/* Col 4: Contacto */}
            <div className="space-y-2.5">
              <p className="text-[10px] font-bold uppercase tracking-widest mb-4" style={{ color: '#35C493' }}>Contacto</p>
              <a href="mailto:contacto@espot.do" className="block text-sm link-muted" style={{ color: '#6B7280' }}>contacto@espot.do</a>
              <a href="tel:+18295481998" className="block text-sm link-muted" style={{ color: '#6B7280' }}>+1 (829) 548-1998</a>
              <p className="text-sm" style={{ color: '#6B7280', lineHeight: 1.5 }}>Calle Caonabo No. 42, Gazcue<br/>Dist. Nacional, Rep. Dominicana</p>
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-5"
            style={{ borderTop: '1px solid #E8EEE9' }}>

            {/* Logo + pago seguro + logos */}
            <div className="flex flex-col sm:flex-row items-center gap-4">
              <Link href="/"><img src="/logo-dark.svg" alt="EspotHub" style={{ height: 18, width: 'auto' }} /></Link>
              <div className="flex items-center gap-2 flex-wrap justify-center">
                <span style={{ fontSize: 10, color: '#9CA3AF', fontWeight: 600 }}>Pago seguro</span>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/azul-logo.jpg" alt="Azul" style={{ height: 16, width: 'auto', borderRadius: 3 }} />
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/visa-logo.jpg" alt="Visa" style={{ height: 16, width: 'auto', borderRadius: 3 }} />
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/mastercard-logo.svg" alt="Mastercard" style={{ height: 16, width: 'auto' }} />
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/verified-by-visa.png" alt="Verified by Visa" style={{ height: 14, width: 'auto' }} />
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/mastercard-id-check.png" alt="Mastercard ID Check" style={{ height: 14, width: 'auto' }} />
              </div>
            </div>

            {/* Copyright + redes */}
            <div className="flex items-center gap-3">
              <span style={{ fontSize: 11, color: '#9CA3AF' }}>© 2026 ESPOT, S.R.L.</span>
              {[
                { href: 'https://www.instagram.com/espot.do/', label: 'IG', icon: 'M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z' },
                { href: 'https://www.facebook.com/espot.do/', label: 'FB', icon: 'M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z' },
                { href: 'https://www.linkedin.com/company/espotdo/', label: 'LI', icon: 'M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 23.2 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z' },
              ].map(s => (
                <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.label}
                  className="w-7 h-7 rounded-lg flex items-center justify-center transition-all"
                  style={{ background: '#F4F6F5', color: '#9CA3AF' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = '#35C493'; (e.currentTarget as HTMLElement).style.color = '#fff' }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = '#F4F6F5'; (e.currentTarget as HTMLElement).style.color = '#9CA3AF' }}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><path d={s.icon}/></svg>
                </a>
              ))}
            </div>
